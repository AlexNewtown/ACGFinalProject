...
bool Sphere::intersect(const Ray &r, Hit &h) const {
  ...
  double a,b,c,d;
  glm::vec3 dir, ori, hitpoint, norm;
  dir = r.getDirection();
  ori = r.getOrigin();
  a = 1;
  b = glm::dot(dir,ori-center);
  c = glm::dot(ori-center,ori-center) - pow(radius,2);
  d = pow(b,2)-c;
  ...
  sol = (-b-sqrt(d));
  if (sol > 0.0) {
    hitpoint = ori+dir*sol;
    norm = (hitpoint-center)/radius;
    h.set(sol,material,norm);
    return true;
  }
