...
void PhotonMapping::TracePhoton(const glm::vec3 &position, const glm::vec3 &direction, 
  ...
  if (iter != 0) {
    Photon photon(pos, direction, energy, iter);
    kdtree->AddPhoton(photon);
  }
  if (iter < args->num_bounces) {
    Material *hitMaterial = hit.getMaterial();
    glm::vec3 diffuse = hitMaterial->getDiffuseColor();
    glm::vec3 reflective = hitMaterial->getReflectiveColor();
    glm::vec3 new_energy = diffuse * energy;
    glm::vec3 normal = hit.getNormal();
    /*if (glm::length(new_energy) > 0.00001) {
      glm::vec3 new_direction = RandomDiffuseDirection(normal);
      TracePhoton(pos, new_direction, new_energy, iter+1);
    }*/
    new_energy = reflective * energy;
    if (glm::length(new_energy) > 0.00001) {
      glm::vec3 new_direction = MirrorDirection(normal, direction);
      TracePhoton(pos, new_direction, new_energy, iter+1);
    }
  }
