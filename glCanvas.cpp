...
glm::vec3 GLCanvas::TraceRay(double i, double j) {
  ...
  //Stratifies the sampling into largest grid
  int gridSize = (int)sqrt(args->num_antialias_samples);
  int gridSquare = square(gridSize);
  float columnStep = float(1)/float(2*gridSize);
  for (int m=0;m<gridSize;m++) {
    float rowStep = float(1)/float(2*gridSize);
    for (int n=0;n<gridSize;n++) {
      double x = (i-args->width*(float(1)/float(gridSquare) + rowStep))/double(max_d)+float(1)/float(gridSquare) + rowStep;
      double y = (j-args->height*(float(1)/float(gridSquare) + columnStep))/double(max_d)+float(1)/float(gridSquare) + columnStep;
      TraceRayHelper(x,y,color,gridSize,max_d);
      rowStep += float(1)/float(gridSize);
    }
    columnStep += float(1)/float(gridSize);
  }

  //If the given number of samples is not a perfect root, the rest are assigned randomly
  for (int m=0;m<args->num_antialias_samples-square(gridSize);m++) {
    double x = (i-args->width/2.0)/double(max_d)+0.5;
    double y = (j-args->height/2.0)/double(max_d)+0.5;
    x += (args->rand()-.5)/double(max_d);
    y += (args->rand()-.5)/double(max_d);
    Ray r = camera->generateRay(x,y);
    Hit hit;
    color += (raytracer->TraceRay(r,hit,args->num_bounces))/float(args->num_antialias_samples);
...
void GLCanvas::TraceRayHelper(double x, double y, glm::vec3& color, int gridSize, int max_d) {
  if (args->num_antialias_samples > 1) {
    x += (args->rand()-float(1)/float(2*gridSize))/(double(max_d));
    y += (args->rand()-float(1)/float(2*gridSize))/(double(max_d));
  }
  Ray r = camera->generateRay(x,y);
  Hit hit;
  color += raytracer->TraceRay(r,hit,args->num_bounces)/float(args->num_antialias_samples);
