...
void Radiosity::ComputeFormFactors() {
  ...
  for (int i=0;i<num_faces;i++) {
    for (int j=0;j<num_faces;j++) {
      double pctUnoccluded = 0;
      glm::vec3 normi = mesh->getFace(i)->computeNormal();
      glm::vec3 normj = mesh->getFace(j)->computeNormal();
      for (int k=0;k<args->num_form_factor_samples;k++) {
        if (i==j)
          break;
        glm::vec3 randPti, randPtj;
        if (args->num_form_factor_samples == 1) {
          randPti = mesh->getFace(i)->computeCentroid();
          randPtj = mesh->getFace(j)->computeCentroid();
        } else {
          randPti = mesh->getFace(i)->RandomPoint();
          randPtj = mesh->getFace(j)->RandomPoint();
        }

        glm::vec3 dir = glm::normalize(randPtj-randPti);
        double len = glm::length(randPtj-randPti);
        if (args->num_shadow_samples) {
          Ray ray(randPti,dir);
          Hit hit = Hit();
          raytracer->CastRay(ray,hit,true);
          if (glm::length(randPtj - hit.getT()*dir - randPti) < 0.00001) {
            pctUnoccluded+=std::max(0.0,glm::dot(dir,normi)*glm::dot(-dir,normj)/(M_PI*len*len));
          }
        } else
          pctUnoccluded+=std::max(0.0,glm::dot(dir,normi)*glm::dot(-dir,normj)/(M_PI*len*len));
      }
      pctUnoccluded /= double(args->num_form_factor_samples);
      formfactors[i*num_faces+j] = pctUnoccluded;
    }
...
float Radiosity::Iterate() {
  if (formfactors == NULL)
    ComputeFormFactors();
  assert (formfactors != NULL);
  glm::vec3 radToDistro = getUndistributed(max_undistributed_patch);
  for (int i=0;i<num_faces;i++) {
    if (i==max_undistributed_patch) {
      setUndistributed(i,glm::vec3(0.0));
      continue;
    }
    glm::vec3 rad = radToDistro*formfactors[max_undistributed_patch+num_faces*i];
    setRadiance(i,(mesh->getFace(i)->getMaterial()->getDiffuseColor()*rad)+getRadiance(i));
    setUndistributed(i,(mesh->getFace(i)->getMaterial()->getDiffuseColor()*rad)+getUndistributed(i));
    setAbsorbed(i,(glm::vec3(1.0)-mesh->getFace(i)->getMaterial()->getDiffuseColor())*rad + getAbsorbed(i));

  }
  findMaxUndistributed();
  return glm::length(radToDistro);
